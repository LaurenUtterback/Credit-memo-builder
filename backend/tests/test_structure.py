...
# Porter memo hid it by having every field populated).
...
def test_extraction_accepts_formatted_money_and_percents():
    ed = StructureExtraction(salary="$1,443,116", loan_amount="2,925,000",
                             interest_rate="15%", origination_fee_pct="4.0 %")
    assert ed.salary == 1_443_116
    assert ed.loan_amount == 2_925_000
...
def test_capacity_is_measured_against_banked_cash_not_the_thinnest_month():
    ...
    caps a $1.44m NFL salary near $19,000, because January carries one check."""
    inputs = nfl_inputs(salary=1_443_116.0, other_debt_annual=327_768.0,
                        funding_date=date(2026, 8, 1), target_term_months=6,
                        contract_end=date(2027, 1, 4), loan_amount=1.0)
    cap = st.max_supportable_loan(inputs)
    assert cap > 300_000, f"capacity {cap:,.0f} is implausibly low for a $1.4m salary"
...
def test_terms_are_held_at_the_ltc_policy_limit_when_policy_binds():
    ...
    assert t.policy_cap == pytest.approx(1_443_116.0 * LTC_MAX_PCT / 100, abs=1000)
...
def test_debt_service_uses_the_memos_own_rows():
    ...
    ed = Extraction(mortgage_payments=293_964, auto_payments=24_204,
                    interest_principal_loans=9_600)
    ds = st.debt_service_from_memo(ed, date(2026, 8, 1))
    assert ds["annual"] == pytest.approx(327_768.0)
